...
function playNote(noteArray, durationArray, instrument)
    ...
    for i = 1:length(noteArray)
        ...
        % If it's a single string (one note or rest), wrap in cell array
        ...
        for k = 1:length(notes)
            ...
            if f > 0
                tone = tone + waveform(instrument, freq, duration, fs, t);
            end
        end

        % Normalize so chords don't clip
        waveform = waveform / max(abs(waveform) + eps);
